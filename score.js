...
function point(teamNum) {
	...
	if(teamNum == 1) {
		...
		if((team1Score == capAt) || (team1Score >= gamesTo && team1Score - team2Score >= winBy)) {
			// Team one wins (submit game score and increment match score and reset game score)
			game(1);
		} else {
	...
	// Otherwise team two scored
		...
		if((team2Score == capAt) || (team2Score >= gamesTo && team2Score - team1Score >= winBy)) {
			// Team two wins (submit game score and increment match score and reset game score)
			game(2);
		} else {
			// Otherwise check for and apply side switching
...
* Function to 
********************************************************************************/
function determineStatus(gameID) {
...
function createJSONobjects() {
	if (xmlHttp.readyState == 4 || xmlHttp.readyState == "complete") {
		...
		if(gamesWonByTeam1   != 0 || gamesWonByTeam2   != 0 || 
		currentScoreTeam1 != 0 || currentScoreTeam2 != 0) {
...
********************************************************************************/
function checkRemoteCommandQueue() {
	console.log("checking queue");
	loadFile("popDBcmd.php","VAR", processCommand);
}
